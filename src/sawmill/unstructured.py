...
class RestructuredFileData(object):
    """
    ...
    def _extract_entries(
        ...
        self.entries = pd.DataFrame({"entry": entries, "line_numbers": line_indices})
    ...
    def _extract_columns(
        ...
        def extract_columns(self, entries: pd.DataFrame = None):
            """
            Extracts each entry, raw file line numbers, and metadata from each row's complex string in the 'entries' DataFrame and creates a new DataFrame with the
